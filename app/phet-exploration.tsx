...
export function PhETExploration({page,sim,onChange,answers,onAnswer}:{page:number;sim:SimState;onChange:(s:SimState)=>void;answers:Record<string,AnswerRecord>;onAnswer:(id:string,a:AnswerRecord)=>void}){
 const states=page===0;const questions=states?stateQuestions:diffusionQuestions;const active=sim.condition;const q=questions[active];const a=answers[q.id]||freshRecord();const [loaded,setLoaded]=useState(false);const [slow,setSlow]=useState(false);const [frameVersion,setFrameVersion]=useState(0);const [instruction,setInstruction]=useState(0);
 useEffect(()=>{if(loaded)return;const timer=setTimeout(()=>setSlow(true),15000);return()=>clearTimeout(timer);},[loaded,frameVersion]);
 const choose=(i:number)=>{setInstruction(0);onChange({...sim,condition:i,prediction:'',shown:false});};
 ...
 <section className="step-guide" aria-label="目前操作指示">
 <strong>操作指示 {instruction+1} / 3</strong>
 <p key={q.id+'-'+instruction}>{(states?['在左側模擬選擇「狀態」，保持物質種類不變。',`點選「${['固態','液態','氣態'][active]}」，觀察分子間距、排列及移動。`,'選出符合觀察的描述，再提交答案。']:[`重設左側模擬，保留分隔器；將藍色粒子設為 ${diffusionSetups[active].blue}、紅色粒子設為 ${diffusionSetups[active].red}。`,'先留下「我的預測」。','移除分隔器並播放，分別追蹤藍色與紅色粒子，再回答觀察題。'])[instruction]}</p>
 {!states&&instruction===1&&<Pick label="我的預測：移除分隔器後會發生什麼？（不計分）" options={['兩種粒子都會逐漸散開、混合','只有數量較多的那種粒子會散開','兩種粒子數量相同時，就不會移動']} value={sim.prediction} onChange={prediction=>onChange({...sim,prediction})}/>}
 {instruction===2&&<>{!states&&sim.prediction&&<p>我的預測：{sim.prediction}</p>}<QuestionCard key={q.id} question={q} record={a} onChange={record}/></>}
 <div className="controls"><Button variant="outline" disabled={instruction===0} onClick={()=>setInstruction(i=>Math.max(0,i-1))}>上一指示</Button><Button variant="outline" disabled={instruction===2||(!states&&instruction===1&&!sim.prediction)} onClick={()=>setInstruction(i=>Math.min(2,i+1))}>下一指示</Button></div>
 </section>
 </OperationLayout>
