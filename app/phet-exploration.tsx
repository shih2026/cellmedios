...
export function PhETExploration({page,sim,onChange,answers,onAnswer}:{page:number;sim:SimState;onChange:(s:SimState)=>void;answers:Record<string,AnswerRecord>;onAnswer:(id:string,a:AnswerRecord)=>void}){
 ...
 <p>{states?'先在下方模擬選擇「狀態」，維持同一種物質，依序切換固態、液態與氣態。每次操作後，回答下方的觀察題。':'每次先重設下方模擬，設定指定的兩種粒子數量，其他條件維持不變。先留下預測，再移除分隔器，觀察一段時間後作答。'}</p>
 ...
 <div className="phet-instructions"><strong>本次操作</strong><ol>{states?<><li>在模擬中選擇「狀態」畫面，保持物質種類不變。</li><li>點選「{['固態','液態','氣態'][active]}」，觀察分子間距、排列及移動。</li><li>在模擬下方選出符合你觀察到的描述。</li></>:<><li>重設模擬，保留分隔器；將藍色粒子設為 {diffusionSetups[active].blue}、紅色粒子設為 {diffusionSetups[active].red}。</li><li>先回答下方的「我的預測」。</li><li>移除分隔器並播放，分別追蹤藍色與紅色粒子，再回答觀察題。</li></>}</ol><p className="small">上方按鈕切換觀察任務；請在模擬內自行設定對應條件。</p></div>
 ...
 <div className="phet-frame"><iframe key={frameVersion} src={states?urls.states:urls.diffusion} title={states?'PhET 物質三態：基礎，繁體中文互動模擬':'PhET 擴散，繁體中文互動模擬'} allowFullScreen onLoad={()=>setLoaded(true)} onError={()=>setSlow(true)}/></div>
